...
<body>
    <div class="d-flex" id="wrapper">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12 p-5 table-responsive">                      
                        <h4>ISSUER TYPE</h4>
                        <div  class ="add-butt p-3"><a  href="javascript:void(0);" data-toggle="modal" data-target="#issuertypeadd-modal"><button type="button" class="btn btn-primary b-btn" id="addissuertype">ADD<i class="fa fa-plus pl-2" aria-hidden="true"></i></button></a></div>
                        <table id="issuertype-table" class="table table-bordered text-nowrap">
                            ...
                            <tbody>
                                @foreach($allissuertypeinfo as $issuertypeinfo)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$issuertypeinfo->issuer_code}}</td>
                                    <td>{{$issuertypeinfo->issuer_name}}</td>
                                    <td>{{$issuertypeinfo->created_on}}</td>
        ...
        <div class="modal fade" id="issuertypeadd-modal">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header text-center d-block p-2 border-bottom-0 bg-dark text-light">
                    <h4 class="modal-title">ADD ISSUER TYPE</h4>
                    <button type="button" class="close position-absolute" style="right: 15px; top: 8px;" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div id="issuermsg"></div>
                        <form action="#"  method="post" enctype = "multipart/form-data" class="w-sm-100 w-auto mx-auto">
                            @csrf
                            <div class="form-group w-100">
                                <label for="issuertype-code">Issuer Type Code*:</label>
                                <input type="text" class="form-control" id="issuertype-code" name="issuertypecode" placeholder="Enter Issuer Type code" value="{{ old('issuertypecode') }}" required>
                            </div>
                            <div class="form-group w-100">
                                <label for="issuertype-name">Issuer Type*:</label>
                                <input type="text" class="form-control" id="issuertype-name" name="issuertypename" placeholder="Enter Issuer Type" value="{{ old('issuertypename') }}" required>
                            </div>
                            <div class="d-flex justify-content-center py-4">
								<button type="button" id="issuertypeaddbutt" class="btn btn-primary b-btn">ADD</button>
							</div>
                        </form>
                    </div>
                </div>
