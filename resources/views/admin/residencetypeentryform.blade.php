...
<body>
    <div class="d-flex" id="wrapper">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12 p-5 table-responsive">                      
                        ...
                        <div  class ="add-butt p-3"><a  href="javascript:void(0);" data-toggle="modal" data-target="#residencetypeadd-modal"><button type="button" class="btn btn-primary b-btn" id="addresidencetype">ADD<i class="fa fa-plus pl-2" aria-hidden="true"></i></button></a></div>
                        <table id="residencetype-table" class="table table-bordered text-nowrap">
                            ...
                            <tbody>
                                @foreach($allresidencetypeinfo as $residencetypeinfo)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$residencetypeinfo->residence_code}}</td>
                                    <td>{{$residencetypeinfo->residence_name}}</td>
                                    <td>{{$residencetypeinfo->created_on}}</td>
        ...
        <div class="modal fade" id="residencetypeadd-modal">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header text-center d-block p-2 border-bottom-0 bg-dark text-light">
                    <h4 class="modal-title">ADD RESIDENCE TYPE</h4>
                    <button type="button" class="close position-absolute" style="right: 15px; top: 8px;" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div id="resimsg"></div>
                        <form action="#"  method="post" enctype = "multipart/form-data" class="w-sm-100 w-auto mx-auto">
                            ...
                            <div class="form-group w-100">
                                <label for="resitype-code">Residence Type Code*:</label>
                                <input type="text" class="form-control" id="resitype-code" name="residencetypecode" placeholder="Enter Residence Type code" value="{{ old('residencetypecode') }}" required>
                            </div>
                            <div class="form-group w-100">
                                <label for="resitype-name">Nature of Work*:</label>
                                <input type="text" class="form-control" id="resitype-name" name="residencetype" placeholder="Enter Residence Type" value="{{ old('residencetype') }}" required>
                            </div>
                            <div class="d-flex justify-content-center py-4">
								<button type="button" id="residencetypeaddbutt" class="btn btn-primary b-btn">ADD</button>
							</div>
                        </form>
                    </div>
                </div>
