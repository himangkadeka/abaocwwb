...
<body>
    <div class="d-flex" id="wrapper">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12 p-5 table-responsive">                      
                        ...
                        <div  class ="add-butt p-3"><a  href="javascript:void(0);" data-toggle="modal" data-target="#natureofworkadd-modal"><button type="button" class="btn btn-primary b-btn" id="addnatureofwork">ADD<i class="fa fa-plus pl-2" aria-hidden="true"></i></button></a></div>
                        <table id="natureofwork-table" class="table table-bordered text-nowrap">
                            ...
                            <tbody>
                                @foreach($allnatureofworkinfo as $natureofworkinfo)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$natureofworkinfo->nature_of_work_code}}</td>
                                    <td>{{$natureofworkinfo->nature_of_work}}</td>
                                    <td>{{$natureofworkinfo->created_on}}</td>
        ...
        <div class="modal fade" id="natureofworkadd-modal">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header text-center d-block p-2 border-bottom-0 bg-dark text-light">
                    <h4 class="modal-title">ADD NATURE OF WORK</h4>
                    <button type="button" class="close position-absolute" style="right: 15px; top: 8px;" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div id="nowmsg"></div>
                        <form action="#"  method="post" enctype = "multipart/form-data" class="w-sm-100 w-auto mx-auto">
                            ...
                            <div class="form-group w-100">
                                <label for="now-code">Nature of Work Code*:</label>
                                <input type="text" class="form-control" id="now-code" name="natureofworkcode" placeholder="Enter Nature of work code" value="{{ old('natureofworkcode') }}" required>
                            </div>
                            <div class="form-group w-100">
                                <label for="now-name">Nature of Work*:</label>
                                <input type="text" class="form-control" id="now-name" name="natureofwork" placeholder="Enter Nature of Work" value="{{ old('natureofwork') }}" required>
                            </div>
                            <div class="d-flex justify-content-center py-4">
								<button type="button" id="natureofworkaddbutt" class="btn btn-primary b-btn">ADD</button>
							</div>
                        </form>
                    </div>
                </div>
