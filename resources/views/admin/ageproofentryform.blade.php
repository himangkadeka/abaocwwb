...
<body>
    <div class="d-flex" id="wrapper">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12 p-5 table-responsive">                      
                        <h4>AGE PROOF</h4>
                        <div  class ="add-butt p-3 "><a  href="javascript:void(0);" data-toggle="modal" data-target="#ageproofadd-modal"><button type="button" class="btn btn-primary b-btn" id="addageproof">ADD<i class="fa fa-plus pl-2" aria-hidden="true"></i></button></a></div>
                        <table id="ageproof-table" class="table table-bordered text-nowrap">
                            ...
                            <tbody>
                                @foreach($allageproofinfo as $ageproofinfo)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$ageproofinfo->age_proof_code}}</td>
                                    <td>{{$ageproofinfo->age_proof_name}}</td>
                                    <td>{{$ageproofinfo->created_on}}</td>
        ...
        <div class="modal fade" id="ageproofadd-modal">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header text-center d-block p-2 border-bottom-0 bg-dark text-light">
                    <h4 class="modal-title">ADD AGE PROOF</h4>
                    <button type="button" class="close position-absolute" style="right: 15px; top: 8px;" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div id="ageproof-msg"></div>
                        <form action="#" method="post" enctype = "multipart/form-data" class="w-sm-100 w-auto mx-auto">
                            @csrf
                            <div class="form-group w-100">
                                <label for="ageproof-name">Age Proof*:</label>
                                <input type="text" class="form-control" id="ageproof-name" name="ageproof" placeholder="Enter Age Proof Name" value="{{ old('ageproof') }}" required>
                            </div>
                            <div class="d-flex justify-content-center py-4">
								<button type="button" id="ageproofaddbutt" class="btn btn-primary b-btn">ADD</button>
							</div>
                        </form>
                    </div>
                </div>
