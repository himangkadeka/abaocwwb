                ...
                <div class="my-5" id="b-homedb">
                    <div class="row">
                        <div class="container-sm ">
                                <h4 class="text-left mb-3 b-latest-data">ADD STATE</h4>
                                <form action="addstate" method="post" class="w-sm-50 w-auto mx-auto">
                                @csrf
                                @if(Session::has('msg'))
               				    ...
               				    @endif
                                    <div class="form-group w-50">
                                        ...
                                        <input type="text" class="form-control" id="state-code" name="statecode" placeholder="Enter State Code as per LGD" value="{{ old('statecode') }}" required>
                                    ...
                                    <div class="form-group w-50">
                                        ...
                                        <input type="text" class="form-control" id="state-name" name="statename" placeholder="Enter State Name" value="{{ old('statename') }}" required>
                                        @if ($errors->has('statename'))
                    				        <span class="text-danger font-weight-bold">{{ $errors->first('statename') }}</span>
               					        @endif
                                    </div>
                        ...
                        <table id="state-table" class="table table-bordered text-nowrap">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{$stateinfo->state_code}}</td>
                                    <td>{{$stateinfo->state_name}}</td>
                                    <td>{{$stateinfo->created_on}}</td>
