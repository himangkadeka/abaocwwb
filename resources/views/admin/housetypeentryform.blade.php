...
<body>
    <div class="d-flex" id="wrapper">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12 p-5 table-responsive">                      
                        <h4>HOUSE TYPE</h4>
                        <div  class ="add-butt p-3"><a  href="javascript:void(0);" data-toggle="modal" data-target="#housetypeadd-modal"><button type="button" class="btn btn-primary b-btn" id="addhousetype">ADD<i class="fa fa-plus pl-2" aria-hidden="true"></i></button></a></div>
                        <table id="housetype-table" class="table table-bordered text-nowrap">
                            ...
                            <tbody>
                                @foreach($allhousetypeinfo as $housetypeinfo)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$housetypeinfo->house_code}}</td>
                                    <td>{{$housetypeinfo->house_type}}</td>
                                    <td>{{$housetypeinfo->created_on}}</td>
        ...
        <div class="modal fade" id="housetypeadd-modal">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div id="housetypemsg"></div>
                        <form action="#"  method="post" enctype = "multipart/form-data" class="w-sm-100 w-auto mx-auto">
                            @csrf
                            <div class="form-group w-100">
                                <label for="housetype-code">House Type Code*:</label>
                                <input type="text" class="form-control" id="housetype-code" name="housetypecode" placeholder="Enter House Type code" value="{{ old('housetypecode') }}" required>
                            </div>
                            <div class="form-group w-100">
                                <label for="house-type">House Type*:</label>
                                <input type="text" class="form-control" id="house-type" name="housetype" placeholder="Enter House Type" value="{{ old('housetype') }}" required>
                            </div>
                            <div class="d-flex justify-content-center py-4">
								        <button type="button" id="housetypeaddbutt" class="btn btn-primary b-btn">ADD</button>
							</div>
                        </form>
                    </div>
                </div>
