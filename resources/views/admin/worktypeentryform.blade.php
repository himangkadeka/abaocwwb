...
<body>
    <div class="d-flex" id="wrapper">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12 p-5 table-responsive">                      
                        <h4>WORK TYPE</h4>
                        <div  class ="add-butt p-3"><a  href="javascript:void(0);" data-toggle="modal" data-target="#worktypeadd-modal"><button type="button" class="btn btn-primary b-btn" id="addworktype">ADD<i class="fa fa-plus pl-2" aria-hidden="true"></i></button></a></div>
                        <table id="worktype-table" class="table table-bordered text-nowrap">
                            ...
                            <tbody>
                                @foreach($allworktypeinfo as $worktypeinfo)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$worktypeinfo->work_type_code}}</td>
                                    <td>{{$worktypeinfo->work_type_name}}</td>
                                    <td>{{$worktypeinfo->created_on}}</td>
        ...
        <div class="modal fade" id="worktypeadd-modal">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div id="worktype-msg"></div>
                        <form action="#"  method="post" enctype = "multipart/form-data" class="w-sm-100 w-auto mx-auto">
                            @csrf
                            <div class="form-group w-100">
                                <label for="worktype-code">Work Type Code*:</label>
                                <input type="text" class="form-control" id="worktype-code" name="worktypecode" placeholder="Enter Work Type code" value="{{ old('worktypecode') }}" required>
                            </div>
                            <div class="form-group w-100">
                                <label for="worktype-name">Work Type*:</label>
                                <input type="text" class="form-control" id="worktype-name" name="worktypename" placeholder="Enter Work Type" value="{{ old('worktypename') }}" required>
                            </div>
                            <div class="d-flex justify-content-center py-4">
								<button type="button" id="worktypeaddbutt" class="btn btn-primary b-btn">ADD</button>
							</div>
                        </form>
                    </div>
                </div>
